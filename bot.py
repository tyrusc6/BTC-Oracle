...
def deep_reflect(trade, outcome, brain, all_history):
    ...
    try:
        ...
        entry     = float(trade.get("btc_price", 0))
        exit_p    = float(trade.get("exit_price", entry))
        move      = round(exit_p - entry, 2)
        rsi       = trade.get("rsi", "?")
        bb        = trade.get("bb_position", "?")
        vol       = trade.get("vol_spike", "?")
        mom       = trade.get("momentum_1h", "?")
        ...
        prompt = """You are the brain of an elite BTC trading bot. You just completed a trade and must now deeply reflect and update your own memory, rules, and strategy. Your memory persists forever — you are getting smarter with every single trade.
...
Now reflect deeply and respond in this EXACT JSON format (no markdown, no backticks):
{
  "new_rule": "one specific rule you are adding or updating based on this trade (be precise with numbers)",
  "win_pattern": "if WIN: describe exact market conditions that led to success, else write null",
  "loss_pattern": "if LOSS: describe exact conditions that caused failure and what to avoid, else write null",
  "memory_note": "a brief market observation to remember forever (max 20 words)",
  "strategy_update": "rewrite your full strategy in 3-5 sentences based on everything you know now",
  "personality_update": "describe your trading personality and style in 2-3 sentences based on your track record"
}"""
        ...
        # fallback basic learning
        signal = trade.get("signal", "")
        rsi = float(trade.get("rsi", 50))
        vol = float(trade.get("vol_spike", 1))
        mom = float(trade.get("momentum_1h", 0))
        bb = float(trade.get("bb_position", 50))
        if outcome == "LOSS":
            if signal == "UP" and rsi > 65:
                brain["rulebook"].append("Avoid UP when RSI > 65 (overbought)")
            elif signal == "DOWN" and rsi < 35:
                brain["rulebook"].append("Avoid DOWN when RSI < 35 (oversold)")
            elif vol < 0.5:
                brain["rulebook"].append("Need stronger volume confirmation before trading")
            brain["loss_patterns"].append(signal + " loss: RSI=" + str(rsi) + " BB=" + str(bb) + " Vol=" + str(vol))
        else:
            brain["win_patterns"].append(signal + " win: RSI=" + str(rsi) + " BB=" + str(bb) + " Vol=" + str(vol))
        brain["total_adjustments"] = brain.get("total_adjustments", 0) + 1
