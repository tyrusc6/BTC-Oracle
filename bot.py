...
# ═══════════════════════════════════════════════════
# MEMORY SYSTEM — smarter than a human
# ═══════════════════════════════════════════════════
...
def deep_reflect(trade, outcome, brain, all_history):
    """Claude deeply reflects on every trade and rewrites its own memory"""
    try:
        ...
        recent_txt = "\n".join([
            t["signal"] + " | RSI:" + str(t.get("rsi","?")) + " | BB:" + str(t.get("bb_position","?")) + " | " + t.get("outcome","?")
            for t in recent
        ])

        current_rules = "\n".join(brain.get("rulebook", [])) or "None yet"
        current_strategy = brain.get("strategy_notes", "") or "None yet"
        current_personality = brain.get("personality", "") or "None yet"

        prompt = """You are the brain of an elite BTC trading bot. You just completed a trade. Reflect deeply and write REAL actionable insights — not just raw numbers.

TRADE JUST COMPLETED:
Signal: """ + signal + """ | Outcome: """ + outcome + """
Entry: $""" + str(entry) + """ | Exit: $""" + str(exit_p) + """ | Price Move: $""" + str(move) + """
RSI: """ + str(rsi) + """ | BB Position: """ + str(bb) + """% | Volume: """ + str(vol) + """x | Momentum: """ + str(mom) + """%

OVERALL RECORD: """ + str(wins) + """W / """ + str(losses) + """L (""" + str(wr) + """% win rate)
...
{
  "new_rule": "Write a specific actionable trading rule learned from this trade. Examples: 'When RSI>70 and BB>80% and momentum negative, strong DOWN signal', 'Avoid DOWN signals when 4h trend is UP and RSI<50', 'High volume spike with 3 green candles = strong UP conviction'. Be precise and useful.",
  "win_pattern": """ + ('"Describe exactly what market conditions made this trade a winner and how to replicate it. Be specific about RSI levels, trends, momentum."' if outcome == "WIN" else '"null"') + """,
  "loss_pattern": """ + ('"Describe exactly why this trade failed and what warning signs to avoid next time. Be specific."' if outcome == "LOSS" else '"null"') + """,
  "memory_note": "One key market insight to remember forever from this trade.",
  "strategy_update": "Rewrite the full trading strategy in 3-5 sentences incorporating everything learned so far. Focus on what actually works.",
  "personality_update": "Describe the trading style in 2-3 sentences: risk tolerance, preferred setups, what to avoid."
}"""
        ...
        if data.get("win_pattern"):
            brain["win_patterns"].append(data["win_pattern"])
        if data.get("loss_pattern"):
        ...
        print("  -> New rule: " + str(data.get("new_rule", "")))
        ...
        if outcome == "LOSS":
            if signal == "UP" and rsi > 65:
                brain["rulebook"].append("Avoid UP signals when RSI above 65 — overbought conditions lead to reversals")
            elif signal == "DOWN" and rsi < 35:
                brain["rulebook"].append("Avoid DOWN signals when RSI below 35 — oversold conditions lead to bounces")
            elif vol < 0.5:
                brain["rulebook"].append("Low volume (under 0.5x) means weak conviction — wait for volume to confirm direction")
            elif signal == "UP" and bb > 80:
                brain["rulebook"].append("Price near BB top (above 80%) with UP signal is risky — likely to reverse")
            elif signal == "DOWN" and bb < 20:
                brain["rulebook"].append("Price near BB bottom (below 20%) with DOWN signal is risky — likely to bounce")
            else:
                brain["rulebook"].append("Market was unpredictable — require stronger multi-timeframe confluence before trading")
            brain["loss_patterns"].append(signal + " failed: RSI was " + str(rsi) + ", BB at " + str(bb) + "%, momentum " + str(mom) + "% — revisit these conditions")
        else:
            if signal == "DOWN" and rsi > 60 and bb > 65:
                brain["win_patterns"].append("DOWN wins when RSI above 60 and price in upper BB — overbought momentum reversal is reliable")
            elif signal == "UP" and rsi < 40 and bb < 35:
                brain["win_patterns"].append("UP wins when RSI below 40 and price in lower BB — oversold bounce is reliable")
            else:
                brain["win_patterns"].append(signal + " won with RSI=" + str(rsi) + " BB=" + str(bb) + "% — these conditions show clear directional bias")
        brain["total_adjustments"] = brain.get("total_adjustments", 0) + 1

def process_commands(brain):
    """Read and execute any commands you sent to the bot"""
    try:
        pending = sb_select("commands", "?status=eq.PENDING&order=created_at.asc&limit=5")
        if not isinstance(pending, list) or len(pending) == 0:
            return
        for cmd in pending:
            print("  -> Processing command: " + cmd["command"])
            prompt = """You are the brain of a BTC trading bot. Your owner just sent you this command:

"" + cmd["command"] + ""

Your current rulebook:
""" + "\n".join(brain.get("rulebook", [])[-20:]) + """

Your current strategy:
""" + brain.get("strategy_notes", "None") + """

Respond in this EXACT JSON format (no markdown):
{
  "response": "acknowledge the command in 1-2 sentences",
  "rule_to_add": "if the command implies a new rule, write it here, else null",
  "rule_to_remove_keyword": "if the command cancels an existing rule, write a keyword to find it, else null",
  "strategy_update": "updated strategy if needed, else null"
}"""
            ...
            try:
                data = json.loads(raw)
                if data.get("rule_to_add"):
                    brain["rulebook"].append("[OWNER] " + data["rule_to_add"])
                if data.get("rule_to_remove_keyword"):
            ...
            print("  -> Command done: " + response_text)
    except Exception as e:
...
def detect_patterns(candles):
    ...
    if len(c) >= 2:
        ...
        if c[-2]["close"]<c[-2]["open"] and c[-1]["close"]>c[-1]["open"] and body>pb: patterns.append("BULL_ENGULF")
        if c[-2]["close"]>c[-2]["open"] and c[-1]["close"]<c[-1]["open"] and body>pb: patterns.append("BEAR_ENGULF")
    if len(c)>=3:
...
def get_market_stats():
    try:
        ...
        d = r.json()
        return {"change_24h": float(d["priceChangePercent"]), "high_24h": float(d["highPrice"]), "low_24h": float(d["lowPrice"]), "vol_24h_m": round(float(d["quoteVolume"])/1e6,1)}
    except: return {}
...
def calc_stats(history):
    ...
    if not resolved: return {"total":0,"wins":0,"losses":0,"win_rate":0,"streak":0,"streak_type":"N/A","last_10":[]}
    ...
    return {
        "total": len(resolved), "wins": wins, "losses": len(resolved)-wins,
        "win_rate": round(wins/len(resolved)*100,1),
        "streak": streak, "streak_type": resolved[0]["outcome"],
...
# ═══════════════════════════════════════════════════
# THE DECISION — ask Claude with full memory
# ═══════════════════════════════════════════════════

def ask_claude(tf1m, tf5m, tf15m, tf1h, tf4h, fg, funding, ob, mstats, news, history, stats, brain):

    # Build memory context
    rulebook_txt = "\n".join(["  " + str(i+1) + ". " + r for i,r in enumerate(brain.get("rulebook",[])[-20:])]) or "  None yet — still learning"
    ...
    strategy = brain.get("strategy_notes","") or "Still developing strategy from data."
    personality = brain.get("personality","") or "Still developing trading style."
    ...
    if resolved:
        hist_txt = "\n".join([
            "  " + t["created_at"][:16] + " | " + t["signal"] + " | RSI:" + str(t.get("rsi","?")) +
            " | BB:" + str(t.get("bb_position","?")) + " | Mom:" + str(t.get("momentum_1h","?")) +
            " | Vol:" + str(t.get("vol_spike","?")) + " | " + t["outcome"]
            for t in resolved
        ])
    ...
    prompt = """You are an elite self-learning BTC trading AI with a persistent memory that grows smarter every trade. You have been trading for a while and have developed your own rules, patterns, and strategy.

Your goal: predict whether BTC will be HIGHER or LOWER in 15 minutes on Kalshi.

═══ YOUR MEMORY & IDENTITY ═══

YOUR STRATEGY (self-written):
""" + strategy + """

YOUR TRADING PERSONALITY:
""" + personality + """

YOUR RULEBOOK (rules you wrote yourself from experience):
""" + rulebook_txt + """

WHAT MADE YOU WIN (patterns you identified):
""" + win_patterns_txt + """

WHAT MADE YOU LOSE (patterns to avoid):
""" + loss_patterns_txt + """

MARKET MEMORY (observations you logged):
""" + memory_txt + """
...
1h:  RSI=""" + str(tf1h["rsi"]) + """ MACD=""" + str(tf1h["macd_hist"]) + """ BB=""" + str(tf1h["bb_pos"]) + """% Mom=""" + str(tf1h["momentum"]) + """% Trend=""" + tf1h["trend"] + """
4h:  RSI=""" + str(tf4h["rsi"]) + """ MACD=""" + str(tf4h["macd_hist"]) + """ BB=""" + str(tf4h["bb_pos"]) + """% Mom=""" + str(tf4h["momentum"]) + """% Trend=""" + tf4h["trend"] + """
...
═══ PERFORMANCE ═══
Win Rate: """ + str(stats["win_rate"]) + """% | """ + str(stats["wins"]) + """W / """ + str(stats["losses"]) + """L | Streak: """ + str(stats["streak"]) + """x """ + stats["streak_type"] + """
Last 10: """ + " ".join(stats["last_10"]) + """
...
2. Think through the timeframes — what is the dominant direction?
3. Consider sentiment — is the market overleveraged?
4. WAIT only when you genuinely cannot determine direction
5. You are accountable to your own rules — follow them
...
def is_kalshi_window():
    """Only signal at :00, :15, :30, :45 — aligned with Kalshi 15-min markets"""
    return datetime.datetime.utcnow().minute % 15 == 0
...
def main():
    ...
    total  = wins + losses
    wr = round(wins/total*100,1) if total>0 else 0
    print("  -> Brain: " + str(len(brain.get('rulebook',[]))) + " rules | " + str(brain.get('total_adjustments',0)) + " reflections | " + str(wr) + "% lifetime win rate")
    ...
    print("  -> $" + str(tf15m["close"]) + " | RSI:" + str(tf15m["rsi"]) + " | Trend:" + tf15m["trend"] + " | BB:" + str(tf15m["bb_pos"]) + "%")
    ...
    # Refresh history after resolution
    history = get_history()
    stats   = calc_stats(history)
    print("  -> Win rate: " + str(stats["win_rate"]) + "% (" + str(stats["wins"]) + "W/" + str(stats["losses"]) + "L)")

    if is_kalshi_window():
        print("  -> Kalshi window! Asking Claude for signal...")
        ...
        mins_until = 15 - (datetime.datetime.utcnow().minute % 15)
