...
def get_btc_data():
    ...
    price = float(requests.get(base + "/ticker/price?symbol=BTCUSDT").json()["price"])
    stats = requests.get(base + "/ticker/24hr?symbol=BTCUSDT").json()
    change_24h = float(stats["priceChangePercent"])
    raw = requests.get(base + "/klines?symbol=BTCUSDT&interval=15m&limit=40").json()
    candles = []
    for c in raw:
...
def calc_win_rate(history):
    ...
    return {"total": len(resolved), "wins": wins, "losses": len(resolved) - wins, "win_rate": round(wins / len(resolved) * 100, 1)}
...
def ask_claude(data, history, stats):
    ...
    if history:
        hist_text = "\n\nRECENT TRADES:\n"
        for t in history[:20]:
            hist_text += "  " + t["created_at"][:16] + " | " + t["signal"] + " | $" + str(t["btc_price"]) + " | " + t.get("outcome", "PENDING") + "\n"
    prompt = "You are an expert BTC short-term trader. Predict if Bitcoin will be HIGHER or LOWER in 15 minutes on Kalshi.\n\nMARKET DATA:\n- Price: $" + str(data["price"]) + "\n- 24h Change: " + str(data["change_24h"]) + "%\n- RSI: " + str(data["rsi"]) + "\n- MACD: " + str(data["macd"]) + "\n- Momentum 1h: " + str(data["momentum_1h"]) + "%\n- Momentum 4h: " + str(data["momentum_4h"]) + "%\n- Volume Spike: " + str(data["vol_spike"]) + "x\n- Bollinger Position: " + str(data["bb_position"]) + "%\n- Last 3 Candles: " + " -> ".join(data["last_3_candles"]) + "\n\nRECORD: " + str(stats["total"]) + " trades | " + str(stats["win_rate"]) + "% win rate | " + str(stats["wins"]) + "W " + str(stats["losses"]) + "L" + hist_text + "\n\nReply with only one word: UP, DOWN, or WAIT"
    msg = claude.messages.create(model="claude-opus-4-6", max_tokens=10, messages=[{"role": "user", "content": prompt}])
...
def main():
    print("==================================================")
    print("  BTC ORACLE  " + datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M") + " UTC")
    print("==================================================")
    print("  -> Pulling BTC data...")
    data = get_btc_data()
    print("  -> $" + str(data["price"]) + " | RSI: " + str(data["rsi"]) + " | Mom: " + str(data["momentum_1h"]) + "%")
