...
def analyze_loss_and_learn(loss_trade, brain):
    ...
    try:
        ...
        if signal == "UP" and rsi > 65:
            ...
            avoid = "Be cautious when volume < 0.7x average but still trade if trend is clear across timeframes"
        elif abs(mom) < 0.05:
...
═══ DECISION RULES ═══
- Signal UP or DOWN when at least 3 of 5 timeframes agree on direction
- Only WAIT if timeframes are completely split or no clear edge exists
- Use lessons learned to avoid SPECIFIC bad setups, not to avoid all trades
- RSI > 78 near BB top = likely reversal DOWN
- RSI < 22 near BB bottom = likely reversal UP
- Low volume alone is NOT enough reason to WAIT — check trend and momentum too
- WAIT should be rare — only when truly no edge exists
- A 55% edge is worth taking — bias toward making a call
